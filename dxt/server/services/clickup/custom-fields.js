...
export class CustomFieldDefinitionService extends BaseClickUpService {
     ...
     */
    async getSpaceCustomFields(spaceId) {
        try {
     ...
     */
    async getFolderCustomFields(folderId) {
        try {
            const response = await this.makeRequest(async () => {
     ...
     */
    async removeCustomFieldValue(taskId, fieldId, useCustomTaskIds = false) {
        try {
            const params = {};
            if (useCustomTaskIds) {
     ...
     */
    async setCustomFieldValue(taskId, fieldId, value, fieldType, useCustomTaskIds = false) {
        try {
            const params = {};
            if (useCustomTaskIds) {
                params.custom_task_ids = 'true';
     ...
     */
    async findCustomFieldByName(listId, fieldName) {
        try {
     ...
     */
    async getWorkspaceCustomFields(spaceId, folderId, listId) {
        try {
            const result = { all: [] };
            ...
            if (listId) {
                ...
                result.list.forEach((f) => fieldMap.set(f.id, f));
            }
            // Get folder fields
            if (folderId) {
                result.folder = await this.getFolderCustomFields(folderId);
                result.folder.forEach((f) => {
