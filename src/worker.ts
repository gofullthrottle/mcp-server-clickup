...
export interface Env {
  ...
  FREE_TIER_TOOLS: string; // Comma-separated list of free tools
  PREMIUM_TIER_TOOLS: string; // Comma-separated list of premium-only tools
}
...
app.post('/auth/api-key', async (c) => {
  ...
  try {
    ...
    // Update user record
    const userService = new UserService(c.env);
    await userService.updateUserConfig(payload.sub as string, {
      has_api_key: true,
      team_id,
    ...
    if (c.env.ENABLE_AUDIT_LOGGING === 'true') {
      ...
      await audit.logAction(payload.sub as string, {
    ...
    switch (event.type) {
      case 'checkout.session.completed': {
        ...
        if (userId) {
          ...
          await c.env.STRIPE_CUSTOMERS.put(
