...
export const advancedTaskTools: Tool[] = [
  {
...
export async function handleAdvancedTaskTool(
  toolName: string,
  args: any
): Promise<any> {
  switch (toolName) {
    case 'clickup_task_create_from_plan': {
      ...
      const tasks = await taskService.planning.createTasksFromPlan(
        validated.list_id,
        validated.plan_text,
        {
    ...
    case 'clickup_task_create_with_scheduling': {
      ...
      const task = await taskService.planning.createTaskWithScheduling(
        validated.list_id,
        {
          name: validated.name,
